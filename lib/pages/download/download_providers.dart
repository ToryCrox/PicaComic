...
/// 标签数据 Provider
final downloadTagsProvider = FutureProvider.autoDispose<List<TagInfo>>((ref) async {
...
final comicUserTagsProvider = FutureProvider.autoDispose<Map<String, List<String>>>((ref) async {
  // 监听下载变化，重新加载标签映射
  ref.watch(allDownloadedComicsProvider.future);
...
/// 更新下载类型筛选
void updateDownloadTypeFilter(WidgetRef ref, String pageId, DownloadType? type) {
  ref.read(downloadPageStateProvider(pageId).notifier).update((state) {
...
final filteredComicsProvider = FutureProvider.autoDispose
  ...
  if (pageState.selectedTagIds.isNotEmpty) {
    ...
    bool isFirst = true;
    for (final tagId in pageState.selectedTagIds) {
    ...
    filtered =
        filtered.where((e) => commonComicIds.contains(e.id)).toList();
  }
...
final selectedCountProvider =
  ...
  return ref.watch(
    downloadPageStateProvider(pageId).select((state) => state.selectedIds.length),
  );
});
...
final filteredTagsProvider = FutureProvider.autoDispose.family<List<TagInfo>, String>((ref, pageId) async {
  ...
  if (isFiltering && filteredComics.isNotEmpty) {
      ...
      tags = tagCountMap.entries.map((e) {
        try {
...
List<String> getUserTags(DownloadedItem item, Map<String, List<String>> userTagsMap) {
  return userTagsMap[item.id] ?? [];
}

List<String> getOriginalTags(DownloadedItem item, Map<String, List<String>> userTagsMap) {
...
List<String> getRawTags(DownloadedItem item, Map<String, List<String>> userTagsMap) {
  final userTags = getUserTags(item, userTagsMap);
  if (userTags.isEmpty) {
